...
while True:
 hostName = os.popen('ssh tomcat@10.203.20.2 hostname 2>/dev/null').read().strip()
 ip =  os.popen('ssh tomcat@10.203.20.2 hostname -I 2>/dev/null | awk \'{print $1}\'').read().strip()

 numCpu = os.popen('ssh tomcat@10.203.20.2 nproc 2>/dev/null').read().strip()
 cpuUse = os.popen('ssh tomcat@10.203.20.2 vmstat 1 3 2>/dev/null | tail -1 | awk \'{print 100 - $15}\'').read().strip()

 numMem = os.popen('ssh tomcat@10.203.20.2 free 2>/dev/null | grep "Mem:" | awk \'{print $2}\'').read().strip()
 numMem = round(int(numMem)/1000000,0)
 memUse = os.popen('ssh tomcat@10.203.20.2 free 2>/dev/null | grep "Mem:" | awk \'{print $3}\'').read().strip()
 ...
 print(f'RWD : Hostname = {hostName}')
 print(f'RWD : IP = {ip}')
 print(f'RWD : Number of CPUs = {numCpu}')
 print(f'RWD : CPU used  = {cpuUse}')
 print(f'RWD : Memory(RAM) allocated  = {numMem}GB')
 print(f'RWD : Memory(RAM) use  = {memUse}GB')
 print(f'RWD : Swapsize set  = {swapSize}')
 print(f'RWD : Swapsize used  = {swapUsed}')


 # Create the system info file and open it for write
 stats_file="/home/sysadm/system_info/rwd_system.html"
 os.system('touch '+stats_file+'')
 outfile = open(stats_file, 'w')

 outfile.write('<html><head><meta http-equiv="refresh" content="60"></head><style>table { width:100%;} table, th, td { border: 1px solid black;} th, td {padding: 10px;text-align: left;} th {background-color: darkred;color: white;}</style><table>')
 ...
 outfile.write("\n")
 outfile.write(f'<tr><td style=color:dimgray><h2>IP</h2></td><td style=color:dimgray><h2>{ip}</h2></td><td style=color:darkred><h2>{ip}</h2></td></tr>')
 outfile.write("\n")
 ...
 discard = os.popen('sudo cp /home/sysadm/system_info/rwd_system.html /var/www/html/').read()
 time.sleep(60)
